    ...
    for (Map.Entry<String, String> entry : this.integrationHandlerPluginMap.entrySet()) {
      ...
      if (ModList.get().isLoaded(entry.getKey())) {
        ...
        try {
          ...
          AthenaeumMod.LOGGER.info("[" + this.modId + "] Loaded integration plugin handler: " + entry.getValue());

        } catch (Exception e) {
          AthenaeumMod.LOGGER.error("[" + this.modId + "] Error loading integration plugin handler: " + entry.getValue(), e);
        }
      }
    }
  }
      ...
      if (handler != null) { // handler is null if required mod isn't loaded
        ...
        for (String plugin : entry.getValue()) {
          ...
          try {
            ...
            AthenaeumMod.LOGGER.info("[" + this.modId + "] Executed integration plugin handler [" + handler.getClass()
                .getName() + "] for plugin [" + plugin + "]");

          } catch (Exception e) {
            AthenaeumMod.LOGGER.error("[" + this.modId + "] Error executing integration plugin handler [" + handler.getClass()
